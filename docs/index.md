...
- **Simple module system** - ability to expose functions for use in other modules, and ability to import them from other modules. [**Modules**](syntax#module-expression) are first level values and can be created dynamically.
...
- **Powerful and efficient** built-in **immutable** data structures with full support for [**pattern matching**](pattern-matching), including Sequence, Dictionary and Set.
...
- **Powerful resource management** - automatically [**manage resources**](resource-management) using built-in context manager infrastructure.
